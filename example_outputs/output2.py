...
for row in cur:
    ...
    for group_key, entry in mf_struct.items():
        ...
        if ((float(row[COLUMN_INDEX['cust']]) == float(cust)) if str(row[COLUMN_INDEX['cust']]).replace('.','',1).isdigit() and str(cust).replace('.','',1).isdigit() else (row[COLUMN_INDEX['cust']] == cust)) and ((float(row[COLUMN_INDEX['prod']]) == float(prod)) if str(row[COLUMN_INDEX['prod']]).replace('.','',1).isdigit() and str(prod).replace('.','',1).isdigit() else (row[COLUMN_INDEX['prod']] == prod)):
...
HAVING_CONDITIONS = entry.sum_1_quant > 10
filtered_groups = []
for group_key, entry in mf_struct.items():
    if eval("entry.sum_1_quant > 10"): #this evaluates the having condition, if it satisfies it will be added to 'filtered_groups'
        filtered_groups.append((group_key, entry))
        

print("\n\nProject Output Debugging Table:")

SELECT_ATTRIBUTES = ['cust', 'prod', 'sum_1_quant', 'avg_1_quant', 'sum_2_quant']
# Print table header
