...
for row in cur:
    ...
    for group_key, entry in mf_struct.items():
        ...
        if ((float(row[COLUMN_INDEX['state']]) == float('NY')) if str(row[COLUMN_INDEX['state']]).replace('.','',1).isdigit() and str('NY').replace('.','',1).isdigit() else (row[COLUMN_INDEX['state']] == 'NY')) and row[COLUMN_INDEX['cust']] == cust:
...
for row in cur:
    ...
    for group_key, entry in mf_struct.items():
        ...
        if ((float(row[COLUMN_INDEX['state']]) == float('NJ')) if str(row[COLUMN_INDEX['state']]).replace('.','',1).isdigit() and str('NJ').replace('.','',1).isdigit() else (row[COLUMN_INDEX['state']] == 'NJ')) and row[COLUMN_INDEX['cust']] == cust:
...
for row in cur:
    ...
    for group_key, entry in mf_struct.items():
        ...
        if ((float(row[COLUMN_INDEX['state']]) == float('CT')) if str(row[COLUMN_INDEX['state']]).replace('.','',1).isdigit() and str('CT').replace('.','',1).isdigit() else (row[COLUMN_INDEX['state']] == 'CT')) and row[COLUMN_INDEX['cust']] == cust:
